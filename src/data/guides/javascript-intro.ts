    ...
    chapters: [
      {
        ...
        sections: [
          ...
          {
            heading: "1.3 プリミティブと参照 — コピーの挙動が変わる",
            body: "JavaScript の値は大きく 2 種類に分かれる。\n\nひとつは『プリミティブ』。これは数値・文字列・真偽値といった、それ以上分解できない単純な値のこと (number / string / boolean / null / undefined / symbol / bigint の 7 種類)。「1」 や 「'hello'」 や 「true」 のような、それ自体が 1 個の値だと思えばよい。\n\nもうひとつは『オブジェクト』(object / 配列 / 関数など)。これは複数の値をまとめて入れる『箱』で、変数はその箱そのものではなく『箱の置き場所 (参照)』を持っている。\n\nこの違いが効いてくるのが『別の変数に代入したとき』。プリミティブは値そのものがコピーされる (コピー後は無関係)。オブジェクトは『同じ箱を指す参照』がコピーされるだけなので、片方をいじるともう片方も変わってしまう。React で『state を直接書き換えてはいけない (新しいオブジェクトを作る)』と言われるのは、この『参照が同じだと変更が漏れる』性質が理由。",
            code: "// プリミティブ: 値そのものがコピーされる (以後は無関係)\nlet a = 1\nlet b = a       // a の値 1 がコピーされる\nb = 99\nconsole.log(a)        // 1   ← a は影響を受けない\n\n// オブジェクト: 『同じ箱を指す参照』がコピーされる\nconst x = { n: 1 }\nconst y = x      // x と y は同じ箱を指す\ny.n = 99\nconsole.log(x.n)      // 99  ← x からも変わって見える (同じ箱だから)\n\n// 別の箱として複製したいとき\nconst z = { ...x }              // 浅いコピー (1 階層だけ複製)\nconst deep = structuredClone(x) // 深いコピー (入れ子もまるごと複製)",
            language: "javascript",
            notes: [
              "プリミティブ = それ以上分解できない単純な値 (number / string / boolean / null / undefined / symbol / bigint)。オブジェクト = 値をまとめた『箱』で、変数は箱の場所 (参照) を持つ",
              "浅いコピー ({ ...obj }) は 1 階層だけ複製。入れ子のオブジェクトは参照のままなので、入れ子ごと複製したいなら structuredClone(obj)",
              "structuredClone は比較的新しい標準。動かない環境では JSON.parse(JSON.stringify(obj)) で代替できる (関数や undefined は消える点に注意)",
            ],
          },
          {
            heading: "1.4 truthy / falsy と、比較・デフォルト値の罠",
            body: "JavaScript では if (値) のように、true/false でない値も条件として書ける。このとき『true として扱われる値』を truthy、『false として扱われる値』を falsy と呼ぶ。\n\n覚えるべきは『falsy はこの 7 つだけ』ということ: false / 0 / '' (空文字) / null / undefined / NaN / 0n。これ以外はすべて truthy。とくに注意なのは、空配列 [] や空オブジェクト {} も truthy (中身が空でも『箱は存在する』ので true 扱い) という点。「データが空かどうか」を if (arr) で判定すると、空配列でも true になって失敗する。\n\n次に比較。== は型が違っても『よしなに』変換してから比べるため、0 == false が true になるなど直感に反する結果を生む。これを避けるため、型も含めて厳密に比べる === を常に使う。\n\n最後にデフォルト値。「値が無ければこれを使う」を書くとき、|| は『falsy なら右辺』なので、0 や '' という正当な値まで弾いてしまう。?? (Nullish Coalescing) は『null か undefined のときだけ右辺』なので、0 や '' はそのまま残せる。デフォルト値には ?? を使うのが安全。",
            code: "// falsy はこの 7 つだけ。それ以外は truthy\nBoolean(false)        // false\nBoolean(0)            // false\nBoolean('')           // false (空文字)\nBoolean(null)         // false\nBoolean(undefined)    // false\nBoolean(NaN)          // false\nBoolean(0n)           // false\nBoolean([])           // true  ← 空配列は truthy!\nBoolean({})           // true  ← 空オブジェクトも truthy!\n\n// == は型変換して比べるので罠が多い\n0 == false            // true  (false が 0 に変換される)\n'' == false           // true\nnull == undefined     // true\n0 === false           // false ← === なら型も見るので安全\n\n// デフォルト値: || は 0 / '' まで弾く、?? は null / undefined だけ\nconst port  = inputPort ?? 3000   // inputPort が 0 なら 0 を採用 (正しい)\nconst port2 = inputPort || 3000   // inputPort が 0 でも 3000 になる (罠)",
            language: "javascript",
            notes: [
              "truthy / falsy = if 文などで true / false に読み替えられるときの扱い。falsy は false/0/''/null/undefined/NaN/0n の 7 つだけ、残りは全部 truthy",
              "『配列が空か』は if (arr) ではなく if (arr.length === 0) で判定する (空配列は truthy のため)",
              "比較は === 一択。デフォルト値は『0 や空文字を有効な値として残したいか』で || と ?? を選ぶ (基本は ??)",
              "?? は ?. (オプショナルチェーン: obj?.a?.b) と組み合わせると、null/undefined 安全にアクセスしつつ既定値を与えられる",
            ],
          },
        ],
        keyTakeaways: [
